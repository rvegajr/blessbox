...
import { Tutorial } from '../components/Tutorial.interface';
import { ContextTrigger } from './context-aware-engine';
...
export function initializeTutorials(): void {
  ...
  if (typeof window !== 'undefined' && (window as any).BlessBoxTutorials) {
    const tutorials = new (window as any).BlessBoxTutorials();
    
    // Register all tutorial definitions
    tutorialDefinitions.forEach(tutorial => {
      tutorials.registerTutorial(tutorial);
    });
    
    // Store reference globally
    (window as any).blessboxTutorials = tutorials;
  }
  
  // Initialize context-aware tutorials
  if (typeof window !== 'undefined' && (window as any).ContextAwareTutorials) {
    const contextTutorials = new (window as any).ContextAwareTutorials();
    ...
    // Store reference globally
    (window as any).contextTutorials = contextTutorials;
  }
}
...
export function trackQRCreateClick(): void {
...
export function trackInviteAttempt(): void {
...
export function markAnalyticsViewed(): void {
