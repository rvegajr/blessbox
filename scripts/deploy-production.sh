...
# 🚀 BlessBox Production Deployment Script
# This script automates the production deployment process

set -e  # Exit on any error

echo "🚀 Starting BlessBox Production Deployment..."
...
print_status "BlessBox Production Deployment Script"
print_status "======================================"
...
# Step 2: Run tests
print_status "Step 2: Running comprehensive test suite..."

# Run unit tests
print_status "Running unit tests..."
npm run test || {
    print_error "Unit tests failed. Please fix issues before deploying."
    exit 1
}
...
if [ ! -f "blessbox-production.db" ]; then
    ...
    # Run database setup
    npm run db:setup || {
        print_warning "Database setup failed. Please check database configuration."
    }
    
    # Seed production data
    print_status "Seeding production data..."
    npm run db:seed || {
        print_warning "Database seeding failed. Please check database configuration."
    }
fi
...
if [ "$choice" != "5" ]; then
    ...
    # This would need to be customized based on the deployment URL
